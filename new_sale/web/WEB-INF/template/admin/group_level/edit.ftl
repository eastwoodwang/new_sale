...
[#include "/admin/common/navbar.ftl"]
    ...
    <div id="page-wrapper" class="gray-bg dashbard-1">
        ...
        <div class="row wrapper border-bottom white-bg page-heading">
            <div class="col-lg-10">
                <h2>${message("admin.group.level")}</h2>
                <ol class="breadcrumb">
                    <li>
                    ${message("admin.title.group")}
                    </li>
                    <li class="active">
                        <strong>${message("admin.group.level.edit")}</strong>
                    </li>
                </ol>
            </div>
            <div class="col-lg-2">
                    ...
                    <div class="ibox float-e-margins">
                        <div class="ibox-content">
                            <form id="inputForm" method="post" class="form-horizontal" action="update.jhtml">
                                <input type="text" hidden="hidden" name="id", value="${groupLevelDTO.id}">
                                <div class="form-group">
                                    <label class="col-sm-2 control-label">${message("admin.group.levelName")}</label>
                                    <div class="col-sm-4"><input id="levelName" name="groupLevelName" value="${groupLevelDTO.groupLevelName}" type="text" class="form-control" required/></div>
                                </div>

                                <div class="form-group">
                                    <label class="col-sm-2 control-label">${message("admin.group.minRange")}</label>
                                    <div class="col-sm-4"><input id="minRange" name="minRange" value="${groupLevelDTO.minRange}" type="number" class="form-control" required/></div>
                                </div>

                                <div class="form-group">
                                    <label class="col-sm-2 control-label">${message("admin.group.maxRange")}</label>
                                    <div class="col-sm-4"><input id="maxRange" name="maxRange" value="${groupLevelDTO.maxRange}" type="number" class="form-control" required/></div>
                                </div>

                                <div class="form-group">
                                    <label class="col-sm-2 control-label">${message("admin.group.discountRate")}</label>
                                    <div class="col-sm-4"><input id="discountRate" name="discountRate" value="${groupLevelDTO.discountRate}" type="number" class="form-control" required/></div>
                                </div>

                                <div class="form-group">
                                    <label class="col-sm-2 control-label">${message("admin.group.priority")}</label>
                                    <div class="col-sm-4"><input id="priority" name="priority" value="${groupLevelDTO.priority}" type="number" class="form-control" required/></div>
                                </div>
...
<script>
    $(document).ready(function(){
        ...
        $inputForm.validate({
            ...
            messages: {
                groupLevelName: "团队等级名不能为空",
                minRange: "请设置团队最低销售值",
                maxRange: "请设置团队最高销售值",
                priority: "请设置团队等级排名",
                discountRate: "请设置团队销售返点率,返点率范围为0-100",
            }
        });
